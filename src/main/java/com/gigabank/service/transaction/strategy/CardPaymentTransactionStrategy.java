...
public class CardPaymentTransactionStrategy implements TransactionStrategy {
     ...
     *
     * <p>Создает {@link TransactionResponseDto} для платежа по карте, заполняя его соответствующими данными, такими как:
    ...
    public void process(AccountRequestDto accountDto, BigDecimal amount, Map<String, String> details) {
        TransactionResponseDto transactionResponseDtoCardPayment = TransactionResponseDto.builder()
                .amount(amount)
        ...
        accountDto.getListTransactionResponseDto().add(transactionResponseDtoCardPayment);
    }
}